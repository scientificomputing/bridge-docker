...
from collections import namedtuple
Dihedral = namedtuple('Dihedral', ['atom1', 'atom2','atom3','atom4'], verbose=verbosity)

# order of dihedral atom is the crystallographic definition. (see glycanstructure.org)
...
#psi
atom1 = "(segid %s and resid %s and name %s)" % (args.isegid5, args.iresid5, args.iname5)
atom2 = "(segid %s and resid %s and name %s)" % (args.isegid6, args.iresid6, args.iname6)
atom3 = "(segid %s and resid %s and name %s)" % (args.isegid7, args.iresid7, args.iname7)
atom4 = "(segid %s and resid %s and name %s)" % (args.isegid8, args.iresid8, args.iname8)
...
phi_trajdata = np.array([(u.trajectory.frame, calc_torsion(dihe_phi)) for ts in u.trajectory])
psi_trajdata = np.array([(u.trajectory.frame, calc_torsion(dihe_psi)) for ts in u.trajectory])
...
with sns.axes_style("white"):
    figure()
    h=sns.jointplot(x=phi_series, y=psi_series, kind="kde")
